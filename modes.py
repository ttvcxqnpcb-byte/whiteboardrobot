...
class FullControlMode(BaseMode):
    ...
    def process_frame(self, frame):
                ...
                if self.ctx.get('bt'):
                    ...
                    if target is not None:
                        ...
                        if box_pts is not None and len(box_pts) == 8:
                            ...
                            dist_front = min(dist_fl, dist_fr) 
                            dist_back = min(dist_br, dist_bl)  
                        else:
                        ...
                        if len(exclude_boxes) > 0:
                            for (ex, ey, ew, eh) in exclude_boxes:
                                # 將禁區矩形向外擴張安全距離，若機器人的導航點踩入此範圍則發出警告
        ...
        # 🌟 [修改] 將 exclude_bboxes 傳入 draw_hud
